...
async function execAgentTool(name, input, env, project, owner, repo, ghHeaders) {
          ...
          if (name === 'write_file') {
            ...
            const payload = { message: input.message || 'create via Falkor agent', content: btoa(unescape(encodeURIComponent(input.content||''))) };
