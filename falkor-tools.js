// falkor-tools v2.1.0 â Asgard-style project hub with Finance, sorting, all info per project
...
const AGENT_TOOLS = [
          ...
          { name:'edit_file', description:"Make a precise, surgical edit to a file. Provide old_string (exact text to find) and new_string (replacement). The old_string must appear EXACTLY ONCE in the file unless replace_all=true. Best for editing large files (e.g. falkor-tools.js itself) without rewriting everything. Auto-commits.",
            input_schema:{ type:'object', properties:{ path:{type:'string'}, old_string:{type:'string', description:'exact text to replace â include enough context to be unique'}, new_string:{type:'string'}, replace_all:{type:'boolean', description:'replace every occurrence (default false)'}, message:{type:'string', description:'commit message'} }, required:['path','old_string','new_string'] } },
          ...
          { name:'racing_comp', description:"Family racing tipping competition leaderboard + today's picks (falkor-sport).",
            input_schema:{ type:'object', properties:{}, required:[] } },
          { name:'pe_advisor', description:"Get PE outdoor lesson advice for today â temperature, UV, conditions, recommendation (falkor-school /pe-advisor).",
            input_schema:{ type:'object', properties:{}, required:[] } },
...
async function execAgentTool(name, input, env, project, owner, repo, ghHeaders) {
          ...
          if (needRepo && !owner && name !== 'cf_deploy_worker') {
            ...
            return { error:'No GitHub repo bound to this project â cannot run '+name+'.' };
          }
          if (name === 'list_files') {
          ...
          if (name === 'read_file') {
            ...
            const MAX = 180000;
            return { path:p, sha, total_bytes: decoded.length, content: decoded.length>MAX ? decoded.substring(0,MAX)+String.fromCharCode(10)+"[truncated at "+MAX+" of "+decoded.length+" bytes â pass start_line/end_line for chunked read]" : decoded };
          }
          ...
          async function browserDispatch(action, input) {
            ...
            return { error: 'browser timeout â is the Falkor Bridge extension installed and connected?' };
          }
          ...
          if (name === 'edit_file') {
            ...
            if (occurrences === 0) return { error:'old_string NOT FOUND in '+p+' â check exact match incl. whitespace/indent' };
            if (occurrences > 1 && !replaceAll) return { error:'old_string matches '+occurrences+' times â add more context to be unique, or set replace_all=true' };
...
<title>Asgard â Project Hub</title>
<style>
...
/* Falkor mascot â individual pose PNGs uploaded to /asset/fk-* */
...
function renderHome(m){
 ...
 const briefLine=el("div",{style:"font-size:12px;color:var(--muted);line-height:1.4"},"Loading todayâs briefingâ¦");
 ...
 // Suggestion chips â clickable shortcuts that pre-fill chat
 const chips=el("div",{style:"display:flex;flex-wrap:wrap;gap:6px;padding:10px 4px"});
 const suggestions=[
  ["ð Todayâs PE outlook", "What's the PE outlook for today? Use the pe-advisor."],
  ["ð Top 5 next actions", "List the top 5 pending project next-actions."],
  ["ð¯ Essendon news", "Anything new for Essendon this week â fixture, ladder, news?"],
  ["ð° Revenue snapshot", "Give me a one-paragraph revenue snapshot across the portfolio."],
  ["ð§ What needs my attention", "What in the portfolio needs my attention right now? Be specific."],
  ["ð¬ Family comp update", "Any updates I should push to the family comp?"]
 ];
 ...
 // Big chat surface â reuse renderChatPane content but stretched
 ...
 const inp=el("input",{type:"text",placeholder:STATE.chatContext?("Ask about "+(STATE.chatContext.name||"this project")+"â¦"):"Ask Falkor anythingâ¦",style:"flex:1;background:var(--panel);border:1px solid var(--border);border-radius:10px;padding:12px 14px;color:var(--text);font-size:14px"});
 const micBtn=el("button",{type:"button",style:"background:var(--panel2);color:var(--text);border:1px solid var(--border);padding:8px 12px;border-radius:10px;cursor:pointer",title:"Hold to record voice"},"ð¤");
 let recorder=null,chunks=[];
 micBtn.addEventListener("mousedown",async()=>{try{const stream=await navigator.mediaDevices.getUserMedia({audio:true});chunks=[];recorder=new MediaRecorder(stream,{mimeType:"audio/webm"});recorder.ondataavailable=e=>{if(e.data.size>0)chunks.push(e.data)};recorder.start();micBtn.style.background="var(--accent)";micBtn.textContent="âº";}catch(e){alert("Mic blocked: "+e.message)}});
 const stop=async()=>{if(!recorder||recorder.state==="inactive")return;recorder.stop();micBtn.style.background="var(--panel2)";micBtn.textContent="ð¤";await new Promise(r=>recorder.onstop=r);recorder.stream.getTracks().forEach(t=>t.stop());const blob=new Blob(chunks,{type:"audio/webm"});micBtn.disabled=true;micBtn.textContent="â¦";try{const r=await fetch("/api/stt",{method:"POST",headers:{"Content-Type":"audio/webm","X-Pin":STATE.agentPin||""},body:blob});const d=await r.json();if(d.text||d.transcript){inp.value=(d.text||d.transcript).trim();form.dispatchEvent(new Event("submit",{cancelable:true}))}else{inp.value=JSON.stringify(d).substring(0,80)}}catch(e){inp.value="STT err: "+e.message}micBtn.disabled=false;micBtn.textContent="ð¤";};
 ...
 form.addEventListener("submit",async(e)=>{
  ...
  STATE.chat.push({role:"assistant",content:"â¦",pending:true});
 ...
 if(STATE.chatContext){
  ...
  ctxBar.appendChild(el("span",{},"â "+(STATE.chatContext.name||"project")));
  const clr=el("button",{style:"background:none;border:none;color:var(--accent);cursor:pointer;font-size:14px;padding:0;line-height:1"},"Ã");
...
function renderChatPane(){
 ...
 form.addEventListener("submit",async(e)=>{
  ...
  btn.disabled=true;
  try{
   // Streaming: route through agent-chat-stream â tokens arrive word-by-word
   const r=await fetch("/api/agent-chat-stream",{method:"POST",headers:{"Content-Type":"application/json","X-Pin":STATE.agentPin||""},body:JSON.stringify({message:text,project:STATE.chatContext||null})});
   if(!r.ok || !r.body){throw new Error("HTTP "+r.status)}
...
function renderTools(m){
 ...
 [["openai","OpenAI TTS (default, free-ish)"],["elevenlabs","ElevenLabs (paid plan needed â voice clone)"]].forEach(([v,t])=>{const o=el("option",{value:v},t);if(localStorage.getItem("falkor.tts.provider")===v)o.selected=true;provSel.appendChild(o)});
...
function renderGuide(m){
 ...
 intro.appendChild(el("div",{},
  ...
  el("div",{style:"font-size:13px;color:var(--muted);line-height:1.5"},"Falkor edits your sites, runs SQL on your D1, deploys workers, drives your browser, and remembers what you tell it. Try the prompts below â copy any of them into the chat panel.")
 ));
 wrap.appendChild(intro);

 const groups = [
  { title:"Edit a project", icon:"\uD83D\uDD27", color:"var(--accent)", prompts:[
    "Open the Bomber Boat project â show me the README",
 ...
 [
  ...
  "Most of your projects auto-deploy on commit â when Falkor edits + commits a file, it ships within a minute.",
...
function renderSchool(m){
 ...
 planBtn.addEventListener("click",async()=>{
  ...
  planOut.appendChild(el("div",{style:"text-align:center;color:var(--muted);font-size:12px;margin-top:6px"},"Generating 5-day planâ¦ (~30s)"));
  ...
  try{
   ...
   if(d.weather_note)planOut.appendChild(el("div",{style:"font-size:11px;color:var(--muted);margin-bottom:8px;background:var(--panel2);padding:6px 10px;border-radius:6px"},"â "+d.weather_note));
   ...
   days.forEach((day,i)=>{
    ...
    const sm=el("summary",{style:"cursor:pointer;font-weight:600;font-size:13px"},"Day "+(i+1)+(day.focus?" â "+day.focus:""));
    dc.appendChild(sm);
    ...
    if(day.curriculum)body.appendChild(el("div",{style:"margin-top:4px;font-size:11px;color:var(--muted)"},"ð "+day.curriculum));
    dc.appendChild(body);
 ...
 fetch("https://falkor-school.luckdragon.io/pe-advisor",{headers:{"X-Pin":PIN}}).then(r=>r.json()).then(d=>{
  ...
  if(cc.temp!=null)grid.appendChild(stat("Temp",cc.temp+"Â°C"));
  if(cc.max!=null)grid.appendChild(stat("Max",cc.max+"Â°C"));
  ...
  const flags=[];
  if(d.sunscreen_required)flags.push("ð§´ Sunscreen");
  if(d.hat_required)flags.push("ð§¢ Hat");
  if(d.water_breaks_required)flags.push("ð§ Water breaks");
  if(flags.length)peBody.appendChild(el("div",{style:"font-size:12px;color:var(--muted);margin-top:6px"},flags.join(" Â· ")));
  if(Array.isArray(d.stop_factors)&&d.stop_factors.length){const w=el("div",{style:"margin-top:10px;padding:10px;background:rgba(239,68,68,.1);border-radius:6px;font-size:12px"});w.appendChild(el("div",{style:"font-weight:600;color:var(--red);margin-bottom:4px"},"â Stop factors"));d.stop_factors.forEach(f=>w.appendChild(el("div",{},"â¢ "+f)));peBody.appendChild(w)}
  if(Array.isArray(d.caution_factors)&&d.caution_factors.length){const w=el("div",{style:"margin-top:8px;padding:10px;background:rgba(245,158,11,.1);border-radius:6px;font-size:12px"});w.appendChild(el("div",{style:"font-weight:600;color:var(--amber);margin-bottom:4px"},"â ï¸ Caution"));d.caution_factors.forEach(f=>w.appendChild(el("div",{},"â¢ "+f)));peBody.appendChild(w)}
...
function renderKBT(m){
 ...
 buildBtn.addEventListener("click",async()=>{
  ...
  buildOut.appendChild(el("div",{style:"text-align:center;color:var(--muted);font-size:12px;margin-top:6px"},"Generating "+roundsIn.value+" rounds Ã "+qprIn.value+" questionsâ¦ (~30s)"));
  ...
  try{
   ...
   if(d.suno_prompt)head.appendChild(el("div",{style:"font-size:11px;color:var(--muted)"},"ðµ Suno prompt available"));
   ...
   (d.rounds||d.pack?.rounds||[]).forEach((rnd,i)=>{
    ...
    (rnd.questions||[]).forEach((q,j)=>{
     ...
     if(q.fun_fact)qd.appendChild(el("div",{style:"color:var(--muted);margin-top:2px;font-style:italic"},"ð¡ "+q.fun_fact));
     rb.appendChild(qd);
    });
 ...
 const liveBody=el("div",{style:"margin-top:8px;font-size:13px"},"Loadingâ¦");
 liveCard.appendChild(liveBody);
 const refresh=async()=>{liveBody.textContent="Loadingâ¦";try{const r=await fetch("https://falkor-kbt.luckdragon.io/game/list",{headers:{"X-Pin":PIN}});const d=await r.json();liveBody.innerHTML="";const games=d.games||(Array.isArray(d)?d:[]);if(games.length===0){liveBody.appendChild(el("div",{style:"color:var(--muted);font-size:12px;text-align:center;padding:20px"},"No live games. Use the build-pack form above to generate a quiz, then host it via the falkor-kbt API."));return}games.forEach(g=>{const gr=el("div",{style:"display:grid;grid-template-columns:1fr auto auto;gap:10px;padding:10px;background:var(--panel2);border-radius:6px;margin-bottom:6px;align-items:center"});gr.appendChild(el("div",{},el("div",{style:"font-weight:600"},g.code||g.id||"?"),el("div",{style:"font-size:11px;color:var(--muted)"},g.theme||g.title||"")));gr.appendChild(el("span",{class:"badge "+(g.status||"").toLowerCase()},g.status||"?"));const view=el("a",{href:"https://falkor-kbt.luckdragon.io/scoreboard/"+(g.code||g.id),target:"_blank",style:"color:var(--accent);font-size:12px;text-decoration:none"},"ðº Scoreboard");gr.appendChild(view);liveBody.appendChild(gr)});}catch(e){liveBody.textContent="Error: "+e.message}};
 const refreshBtn=el("button",{style:"margin-top:8px;background:var(--panel2);color:var(--text);border:1px solid var(--border);padding:6px 12px;border-radius:6px;cursor:pointer"},"â» Refresh");
...
export default {
  async scheduled(event, env, ctx) {
    ...
    const now = new Date();
    try {
      // 09:00 UTC = 7pm Melbourne AEST â AFL daily digest to family
      if (cron === '0 9 * * *') {
      ...
      // 21:00 UTC = 7am Melbourne AEST next-day â daily morning briefing to Paddy
      else if (cron === '0 21 * * *') {
 ...
 log('done. View: /asset/<slug>');
    ...
    if(url.pathname==='/api/route'&&request.method==='POST'){
      ...
      try {
        ...
        const QUOTAS = {
          ...
          'mistral-free': 1000,      // ~33k req/month â 1000/day
        ...
        // Map provider â asgard-ai supported model + provider
    ...
    if(url.pathname.startsWith('/api/sport/afl/')){
      ...
      try {
        ...
        if (sub === 'essendon') {
          const r = await fetch('https://api.squiggle.com.au/?q=games;year='+yr+';team=4&format=json', {headers});
          const d = await r.json();
          const games = d.games||[];
      ...
      const stream = new ReadableStream({
        async start(controller) {
          ...
          try {
            ...
            let system = "You are Falkor â Paddy's personal coding agent embedded in his Asgard project hub. Casual, direct, terse. No fluff, no apologies." + memBlock;
            if (project) {
    ...
    if(url.pathname==='/api/agent-chat'&&request.method==='POST'){
      try {
        ...
        try {
          ...
          const md = await mr.json();
          const mems = md.result?.[0]?.results || [];
          if (mems.length) memBlock = String.fromCharCode(10,10) + 'WHAT YOU REMEMBER ABOUT PADDY (long-term memory â these are important):'+ String.fromCharCode(10) + mems.map(m => '- ['+m.category+'] '+m.fact).join(String.fromCharCode(10));
        } catch(e){}
        system += memBlock;
