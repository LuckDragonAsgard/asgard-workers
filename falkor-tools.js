...
const AGENT_TOOLS = [
          ...
          { name:'cf_deploy_pages', description:"Deploy a Cloudflare Pages project from the asgard-workers GitHub repo. Provide name (Pages project name) and dir (subdirectory in repo containing the build output, default 'public'). Creates the Pages project if it doesn't exist. Returns deployment_id and URL.",
            input_schema:{ type:'object', properties:{ name:{type:'string', description:'Pages project name (e.g. falkor-ui)'}, dir:{type:'string', description:'subdirectory in asgard-workers repo containing static files'} }, required:['name'] } },
...
upBtn.onclick=async()=>{
 ...
 log('done. View: /asset/<slug>');
    ...
    if(url.pathname==='/api/family/tip-history'&&request.method==='GET'){
      try {
        const member = url.searchParams.get('member')||'';
        if (!member) return Response.json({error:'member param required'},{status:400,headers:{...CORS,...NOCACHE}});
        const r = await fetch('https://api.cloudflare.com/client/v4/accounts/'+env.CF_ACCOUNT_ID+'/d1/database/'+env.D1_DB_ID+'/query',{method:'POST',headers:{'Authorization':'Bearer '+env.CF_API_TOKEN,'Content-Type':'application/json'},body:JSON.stringify({sql:'SELECT member, year, round, game_id, tip, points, created_at FROM family_tips WHERE member=? ORDER BY created_at DESC LIMIT 20', params:[member]})});
        const d = await r.json();
        return Response.json({ok:true, member, tips: d.result?.[0]?.results || []}, {headers:{...CORS,...NOCACHE}});
      } catch(e){ return Response.json({error:String(e).substring(0,200)},{status:500,headers:{...CORS,...NOCACHE}}); }
    }
    ...
    if(url.pathname==='/api/falkor/smoke-test'&&request.method==='POST'){
      try {
        ...
        await run('d1_query', async () => {
          const r = await fetch('https://api.cloudflare.com/client/v4/accounts/'+env.CF_ACCOUNT_ID+'/d1/database/'+env.D1_DB_ID+'/query', {
            method:'POST', headers:{'Authorization':'Bearer '+env.CF_API_TOKEN,'Content-Type':'application/json'},
            body: JSON.stringify({sql:'SELECT COUNT(*) as c FROM definitely_not_a_real_table_xyz'}),
